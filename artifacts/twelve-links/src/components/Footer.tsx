...
export default function Footer() {
  // State handles holding active document rendering state
  const [activeDoc, setActiveDoc] = useState<"privacy" | "terms" | null>(null);

  return (
    <footer className="bg-card border-t border-white/10 pt-16 pb-8 relative">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="pt-8 border-t border-white/5 flex flex-col md:flex-row justify-between items-center gap-4 text-xs text-gray-500 font-mono">
          ...
          {/* ── FIXED INTERACTIVE SYSTEM LINKS ── */}
          <div className="flex gap-4">
            <button 
        ...
        {activeDoc && (
          <>
            ...
            <motion.div
              ...
              className="fixed inset-0 bg-black/70 backdrop-blur-md z-50 cursor-crosshair"
            />
            ...
            <motion.div
              ...
              className="fixed inset-y-0 right-0 w-full max-w-2xl bg-card border-l border-white/10 p-8 shadow-2xl z-50 overflow-y-auto flex flex-col"
            >
              {/* Drawer Header */}
              ...
              <div className="flex-1 text-gray-300 space-y-6 text-sm font-light leading-relaxed pr-2 overflow-y-auto">
                {activeDoc === "privacy" ? (
                  /* ── PRIVACY POLICY CONTENT (POPIA ALIGNED) ── */
                  <>
                    ...
                    <div className="space-y-2">
                      ...
                      <p>We only collect data necessary to evaluate, scope, and execute software delivery projects. This includes personal and corporate identifiers provided directly by you via our contact forms (Full Name, Company Name, Official Email Address, Phone Number, Budget Limits, and architectural project constraints).</p>
                    </div>
                    ...
                    <div className="space-y-2">
                      ...
                      <p>Our infrastructure applies enterprise-grade cloud architecture security protocols. Data parameters entered via our contact framework are securely encrypted in transit and protected within isolated cloud repositories to minimize access vector vulnerabilities.</p>
                    </div>

                    <div className="space-y-2">
                      <h4 className="text-white font-bold font-mono text-xs tracking-wider text-[#00FF88] uppercase">4. Third-Party Sharing</h4>
                      <p>12 Links does not sell, lease, or distribute data parameters to marketing conglomerates. Information may only be routed to vital sub-processors (such as automated email notification relays like EmailJS) strictly to fulfill processing requests initialized directly by you.</p>
                    </div>
                  ...
                  /* ── TERMS OF SERVICE CONTENT (COMMERCIAL LEGAL FRAMEWORK) ── */
                  <>
                    <p className="italic text-gray-400 bg-white/5 p-4 rounded-lg border border-white/5">
              ...
              <div className="pt-6 border-t border-white/5 mt-8 flex justify-end">
                <button
                  onClick={() => setActiveDoc(null)}
                  className="px-5 py-2.5 bg-white/5 hover:bg-white/10 rounded-md text-sm text-white font-medium transition-colors cursor-pointer"
                >
                  I Understand
                </button>
              </div>
